...
contract Networking {
    ...
    */
    constructor(address[] memory nodes_addresses, int[] memory clusters, address sink_node) public {
    ...
    */
    ...
    /*
        Send data
    */
    function sendData(address _sender, address _receiver, string memory _data, string memory _date) public {
    ...
    /*
        Send data
    */
    function sendDataToSinkNode(address _cluster_header, string memory _data, string memory _date) public {
        transactions.push(Data({
    ...
    */
    function getNodeDetails(address _nodeAddress) view public returns (address, int) {
    ...
    */
    function getClusterNodesAddresses(int _cluster) view public returns (address[] memory local_addresses) {
        // 40 is the maximum array size
        address[] memory local_addresses = new address[](40);
        int counter = 0;
        for( uint256 i = 0; i < nodes.length; i++ ) {
            if( nodes[i].cluster == _cluster ) {
                local_addresses[i] = nodes[i].nodeAddress;
                counter++;
            }pragma experimental ABIEncoderV2;
...
contract Networking {
    ...
    */
    function getTransactionsDate(address _node) view public returns (string[] memory local_data) {
        local_data = new string[](transactions.length);
        uint256 counter = 0;
        for( uint256 i = 0; i < transactions.length; i++ ) {
    ...
    */
    function getTransactionDetails(string memory _date) public returns (address, address, string memory, string memory) {
        for( uint256 i = 0; i < transactions.length; i++ ) {
            if( equal(transactions[i].date, _date) ) {
